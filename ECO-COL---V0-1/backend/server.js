...
const JWT_SECRET = process.env.JWT_SECRET || 'eco-col-secret-key';
...
app.post('/api/patients', authenticate, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO patients (external_id, name, dni, age, gender, phone, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
...
app.post('/api/studies', authenticate, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO studies (external_id, patient_id, hospital, modality, created_by) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
app.put('/api/studies/:id', authenticate, async (req, res) => {
    try {
        ...
        const updates = ['observations = $1', 'status = $2', 'radiologist_id = $3'];
        const params = [observations, status, req.user.id, req.params.id];
        if (status === 'completed') updates.push('completed_at = CURRENT_TIMESTAMP');
        const result = await pool.query(
            `UPDATE studies SET ${updates.join(', ')} WHERE id = $4 RETURNING *`,
            params
        );
...
app.post('/api/dicom/upload/:studyId', authenticate, upload.single('dicom'), async (req, res) => {
    try {
        ...
        await pool.query('UPDATE studies SET dicom_file_path = $1 WHERE id = $2', [req.file.path, req.params.studyId]);
...
app.get('/api/dicom/download/:studyId', authenticate, async (req, res) => {
    try {
        const result = await pool.query('SELECT dicom_file_path FROM studies WHERE id = $1', [req.params.studyId]);
        if (result.rows.length === 0 || !result.rows[0].dicom_file_path) {
            return res.status(404).json({ error: 'DICOM file not found' });
        }
        res.download(result.rows[0].dicom_file_path);
    } catch (error) {
